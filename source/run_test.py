...
if __name__ == "__main__":
	...
	for i in scales:
		...
		for j in trained_feat:
			...
			if j == 'places' and not os.path.isfile(folder+'/pca/pca_places_s'+i+'_f'+fold+'.pkl'):
				call_pca = 'python '+pcaScript+' --cfg '+places_config_file+' -d 1000 -s '+i+' -f '+fold+' -o '+folder+'/pca/pca_places_s'+i+'_f'+fold+'.pkl'
				callspca.append(call_pca)

			if j == 'imnet' and not os.path.isfile(folder+'/pca/pca_imnet_s'+i+'_f'+fold+'.pkl'):

				call_pca = 'python '+pcaScript+' --cfg '+imnet_config_file+' -d 1000 -s '+i+' -f '+fold+' -o '+folder+'/pca/pca_imnet_s'+i+'_f'+fold+'.pkl'	
				callspca.append(call_pca)
	if len(callspca) > 0:
		print "Performing PCA"
		...
		for j in trained_feat:
			...
			if j == 'places':
				...
				if not os.path.isfile(dict_):
					...
					call_dict = 'python '+dictScript+' --cfg '+places_config_file+' -s '+i+' -o '+os.path.splitext(dict_)[0]+' -f '+fold+' -n '+dsize+' -r '+rsize+' -l '+dlambda+' --pca '+folder+'/pca/pca_places_s'+i+'_f'+fold+'.pkl' + ' --cnn PLACES'
			...
			if j == 'imnet': 
				...
				if not os.path.isfile(dict_):
					...
					call_dict = 'python '+dictScript+' --cfg '+imnet_config_file+' -s '+i+' -o '+os.path.splitext(dict_)[0]+' -f '+fold+' -n '+dsize+' -r '+rsize+' -l '+dlambda+' --pca '+folder+'/pca/pca_imnet_s'+i+'_f'+fold+'.pkl' ' --cnn IMNET'
					callsdict.append(call_dict)
	...
	for i in scales:
		...
		for j in trained_feat:
			...
			if j == 'places' and not os.path.isfile(folder+'/sc_feats/places_s'+i+'_'+mode+'_pca_f'+fold+'_'+meth+'_'+L.replace('.','')+'_d'+ns1+ns2+'_dl'+dlambda.replace('.','')+'.npz'):
				...
				print "call PLACES"	
				call_str = 'python '+poolScript+' --cfg '+places_config_file+' -d '+dict_places+' --pca '+folder+'/pca/pca_places_s'+i+'_f'+fold+'.pkl -o '+folder+'/sc_feats/places_s'+i+'_'+mode+'_pca_f'+fold+'_'+meth+'_'+L.replace('.','')+'_d'+ns1+ns2+'_dl'+dlambda.replace('.','')+' -f '+fold+' -m '+meth+' -j max -l '+L+' -t '+mode.title()+' -s '+i			
			...
			if j == 'imnet' and not os.path.isfile(folder+'/sc_feats/imnet_s'+i+'_'+mode+'_pca_f'+fold+meth+'_'+L.replace('.','')+'_d'+ns1+ns2+'_dl'+dlambda.replace('.','')+'.npz'):
				...
				print "call IMNET"	
				call_str = 'python '+poolScript+' --cfg '+imnet_config_file+' -d '+dict_imnet+' --pca '+folder+'/pca/pca_imnet_s'+i+'_f'+fold+'.pkl -o '+folder+'/sc_feats/imnet_s'+i+'_'+mode+'_pca_f'+fold+meth+'_'+L.replace('.','')+'_d'+ns1+ns2+'_dl'+dlambda.replace('.','')+' -f '+fold+' -m '+meth+' -j max -l '+L+' -t '+mode.title()+' -s '+i
				calls.append(call_str)
	...
	if not os.path.isfile(folder+'/sc_feats/featFull_'+mode+'_pca_f'+fold+meth+'_'+L.replace('.','')+'d_'+ns1+ns2+'_dl'+dlambda.replace('.','')+'.csv'):
	...
	svm_str = 'python '+svmScript+' --input '+folder+'/sc_feats/featFull_'+mode+'_pca_f'+fold+meth+'_'+L.replace('.','')+'d_'+ns1+ns2+'_dl'+dlambda.replace('.','')+'.csv --label '+folder+'/sc_feats/places_s1_'+mode+'_pca_f'+fold+'_'+meth+'_'+L.replace('.','')+'_d'+ns1+ns2+'_dl'+dlambda.replace('.','')+'_label.csv.npz -o '+folder+'/svm/svm-f'+fold+'_lambda'+L.replace('.','')+'_m'+meth+'_d'+ns1+ns2+'_dl'+dlambda.replace('.','')+'.pkl --mode '+mode

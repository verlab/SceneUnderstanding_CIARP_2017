		...
		for j in trained_feat:
			...
			if j == 'places':
				...
				if not os.path.isfile(dict_):
					...
					call_dict = 'python '+dictScript+' --cfg '+places_config_file+' -s '+i+' -o '+os.path.splitext(dict_)[0]+' -f '+fold+' -n '+dsize+' -r '+rsize+' -l '+dlambda+' --pca '+folder+'/pca/pca_places_s'+i+'_f'+fold+'.json'
			...
			if j == 'imnet': 
				...
				if not os.path.isfile(dict_):
					...
					call_dict = 'python '+dictScript+' --cfg '+imnet_config_file+' -s '+i+' -o '+os.path.splitext(dict_)[0]+' -f '+fold+' -n '+dsize+' -r '+rsize+' -l '+dlambda+' --pca '+folder+'/pca/pca_imnet_s'+i+'_f'+fold+'.json'
					callsdict.append(call_dict)
